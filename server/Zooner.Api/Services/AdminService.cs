...
public class AdminService : IAdminService
{
    ...
    public async Task<ApiResponse<List<ShopDto>>> GetAllShopsAsync(ShopVerificationStatus? status = null, string? search = null)
    {
        ...
        var dtos = shops.Select(s => new VendorShopDto
        {
            Id = s.Id,
            OwnerId = s.OwnerId,
            OwnerName = !string.IsNullOrWhiteSpace(s.Owner?.FullName) ? s.Owner.FullName : (!string.IsNullOrWhiteSpace(s.Owner?.Email) ? s.Owner.Email : "Registered Shopper"),
            OwnerEmail = s.Owner?.Email ?? string.Empty,
            OwnerPhone = s.Owner?.PhoneNumber ?? s.Phone ?? string.Empty,
            Name = s.Name,
